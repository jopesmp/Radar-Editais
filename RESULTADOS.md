...
**Dia 1 — Ingestão e exploração.** Mapeei a API do PNCP (duas especificações OpenAPI
distintas: consulta pública e a API de manutenção com GETs públicos escondidos).
Descobri que 5 dos 8 campos pedidos vêm prontos da API; os outros 3 exigem ler o PDF
do edital. Corrigi um bug real no `pncp_client.py` herdado: a deduplicação usava
...
**Dia 2 — Gabarito.** Anotei manualmente 15 documentos, escolhidos por categoria de
...
**Dia 3 — Extração, validação, score, API.** Implementei e testei o pipeline completo
...
**Dia 4 — Evals e ajuste da métrica.** Reescrevi o `eval.py` para medir o pipeline
completo (pós-validação), não só a extração isolada. Corrigi dois problemas na
...
mudar uma linha do sistema, só corrigindo como eu media.
...
modelo gratuito quanto — de forma importante — o esgotamento de cota gratuita que
observei ao longo do dia (ver seção de latência/limitações abaixo). Em execuções
...
conteúdo errado — indicam paráfrase. Conferi manualmente vários casos (ex: o
atestado técnico da NOVACAP) em que o conteúdo extraído estava correto, mas reescrito
com palavras diferentes do gabarito. SequenceMatcher penaliza isso pesadamente; é uma
métrica fraca para texto livre, que mantive por simplicidade (sem dependência nova)
mas documentei como limitação.
...
1. **Documentos em `.zip`/`.rar` são mais comuns do que eu imaginava.** No Dia 1
   identifiquei isso só nos 4 editais da Caixa Econômica Federal. Rodando o pipeline
   em 50 contratações reais no Dia 3, descobri que **cerca de 17 de 50** (34%)
...
3. **O tier gratuito do OpenRouter esgota rápido sob uso intenso.** Com cerca de 130+
   ...
   acurácia entre rodadas no mesmo dia. Mitiguei com uma lista de 5 modelos de
   fallback, mas não eliminei o problema.

4. **Encontrei e corrigi um bug real durante o eval do Dia 4:** um dos modelos de
   fallback (mais fraco) devolveu `exigencias_habilitacao` como uma string solta em
   vez do objeto JSON esperado (`{"valor": ..., "trecho_citado": ...}`). Isso
   derrubava a validação com `AttributeError`. Corrigi com uma checagem de tipo
...
5. **A checagem de citação literal (regra 3.3) começou rigorosa demais** e ajustei
   duas vezes no Dia 4: primeiro para tolerar reformatação de links Markdown e
   ligaduras tipográficas do PDF (ex: "eﬁciência" → "eficiência"), depois para
...
6. **Uma inconsistência de valor ficou sem resolver** (edital do bloquete sextavado,
...
7. **Decisão de score ainda simplificada:** o critério de "localização" usa
   ...
   estado — uma simplificação conhecida, documentada em `DECISOES.md`, que não
   corrigi por falta de tempo.
